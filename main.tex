...
\\
Ciastko jest tylko jedno więc znika po jego zjedzeniu i nie można próbować go zjeść, natomiast duża, nieprzeźroczysta butelka Eliksiru nie musi być pusta po napitku z niej.
\\
...
\lstset{
	style=main,
	keywords=[2]{EatCake, DrinkElixir, BringCake, BringElixir},
}
\begin{lstlisting}[mathescape=true]
initially $\neg$AliceSmall
initially CakeAvailable $\wedge$ ElixirAvailable 
initially $\neg$HatterMad $\wedge$ CatInvisible

(EatCake,Alice) causes $\neg$AliceSmall
impossible (EatCake,CheshireCat)
(EatCake,Hatter) typically causes $\neg$HatterMad if HatterMad
impossible (EatCake,WhiteRabbit)
(EatCake,$\epsilon$) causes $\neg$CakeAvailable
impossible (EatCake,$\epsilon$) if $\neg$CakeAvailable

(DrinkElixir,Alice) causes AliceSmall if ElixirAvailable
(DrinkElixir,CheshireCat) typically causes $\neg$CatInvisible if ElixirAvailable
(DrinkElixir,Hatter) releases HatterMad if $\neg$HatterMad $\wedge$ if ElixirAvailable
(DrinkElixir,WhiteRabbit) causes CakeAvailable if ElixirAvailable
(DrinkElixir,$\epsilon$) releases ElixirAvailable if ElixirAvailable
