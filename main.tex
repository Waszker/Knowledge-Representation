...
\begin{enumerate}
	\item $Res_{0}$ : $A \times \Sigma \to 2^{\Sigma}$ konstruowane na podstawie zdań efektów akcji.
	\[ \forall_{A \in A, \sigma \in \Sigma} Res_{0}(A,\sigma) = \{\sigma' \in \Sigma: (A \textbf{ causes } \alpha \textbf{ if } \pi) \in D \land (\sigma \models \pi) \Rightarrow (\sigma' \models \alpha) \}  \]
	Oznacza to, że $Res_{0}$ konstruuje się bez minimalizacji zmian.
	\item Funkcję $Res^{-}$ wyznacza się stosując minimalizację zmian.
	\item Funkcję $Res^{+} : A \times \Sigma \to 2^{\Sigma}$ spełniającą warunek $\forall_{A \in A, \sigma \in \Sigma}$:
	\[ Res_{0}^{+}(A,\sigma) = \{\sigma' \in Res_{0}(A,\sigma) : (A \textbf{ typically causes } \beta \textbf{ if } \pi) \in D \land (\sigma \models \varphi) \Rightarrow (\sigma' \models \beta) \}  \]
\end{enumerate}

Niech D będzie dziedziną akcji języka $\Omega$ i niech $S=(\Sigma, \sigma_{0}, TesAb, ResN)$ będzie strukturą dla $\Omega$. Mówimy, że S jest modelem D $\leftrightarrow$ spełnione są warunki:
\begin{itemize}
	...
	\item $\forall_{ A \in A, \sigma \in \Sigma } ResN(A, \sigma)$ jest zbiorem tych wszystkich stanów $\sigma' \in Res_{0}^{+}(A, \sigma)$, dla których zbiory $New(A, \sigma, \sigma')$ są minimalne
	\item $\forall_{A \in A, \sigma \in \Sigma} ResAb(A, \sigma) = Res^{-}(A, \sigma) | ResN(A, \sigma)$
\end{itemize}
Warto zwrócić uwagę na to, że skutki \textit{pewne} dla akcji traktowane są jak \textit{typowe}.

\subsubsection{Funkcja przejścia}
Niech $S=(\Sigma,\sigma_{0},ResAb,ResN)$ będzie strukturą dla języka. Konstrukcja funkcji $\Psi_{S} : A^{*} \times \Sigma \to \Sigma$ wygląda następująco:
\begin{itemize}
	\item $\Phi_{S}(\epsilon,\sigma)=\sigma$ gdzie $\epsilon$ oznacza ciąg pusty
	\item jeśli $\Phi_{S}((A_{1}, \dots, A_{n}),\sigma)$ jest określona to
	\[\Phi_{S}((A_{1},\dots,A_{n}),\sigma) \in ResAb(A_{n}, \Phi_{S}(A_{1},\dots,A_{n-1})) \cup ResN(A_{n}, \Phi_{S}(A_{1},\dots,A_{n-1}))\]
\end{itemize}
