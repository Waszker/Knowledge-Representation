...
\begin{enumerate}
	...
	\[ \forall_{a \in A \times W, \sigma \in \Sigma} Res_{0}(a,\sigma) = \{\sigma' \in \Sigma: (a \textbf{ causes } \alpha \textbf{ if } \pi) \in D \land (\sigma \models \pi) \Rightarrow (\sigma' \models \alpha) \}  \]
	Oznacza to, że $Res_{0}$ konstruuje się bez minimalizacji zmian.
	\item Funkcję $Res^{-}$ wyznacza się stosując minimalizację zmian.
	\item Funkcję $Res^{+} : A \times W \times \Sigma \to 2^{\Sigma}$ spełniającą warunek $\forall_{a \in A \times W, \sigma \in \Sigma}$:
...
\begin{itemize}
	...
	\item $\forall_{ a \in A \times W, \sigma \in \Sigma } ResN(a, \sigma)$ jest zbiorem tych wszystkich stanów $\sigma' \in Res_{0}^{+}(a, \sigma)$, dla których zbiory $New(a, \sigma, \sigma')$ są minimalne
	\item $\forall_{a \in A \times W, \sigma \in \Sigma} ResAb(a, \sigma) = Res^{-}(a, \sigma) | ResN(a, \sigma)$
\end{itemize}
