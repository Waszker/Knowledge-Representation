...
\If{$\neg hasGammaBeenAchieved$}
    \State $actionGroups \gets $ krawędzie wychodzące ze $state$ zgrupowane wg par (Akcja, Aktor)
...
\ForAll{$\texttt{startState} \in StatesPi$}
    ...
    \While{$open$ niepusty}
        ...
        \EndIf
        \ForAll{$state2 \in$ stany, do których można (typowo) przejść z $state$}
            \If{$state2 \not\in close$}
                \State dodaj $state2$ do $open$
            \EndIf
        \EndFor
