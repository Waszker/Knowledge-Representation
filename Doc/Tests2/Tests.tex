...
Kwerendy:
\begin{lstlisting}[mathescape=true]
ever accessible $\neg$burned if burned when (TORCH,Nero)
always accessible $\neg$burned if burned when (TORCH,Nero)
\end{lstlisting}
zwracają kolejno $False$ i $True$, co nie powinno mieć miejsca (\textit{ever} nie powinno zwracać prawdy rzadziej, niż \textit{always}).
